...
<script type="text/javascript">
	...
	hello();
      ...
      <div class="rightDiv">
        <br>
      <form name="frm" method="post" action="__APP__/UpdateNews/update/articleId/<?php echo ($articleId); ?>/typeId/<?php echo ($oldTypeId); ?>" >
        <table class="addNewsTb" bgcolor="#E4E9EC" border="0" align="center" width="700">
          <tr>
            <td class="changeBack">新闻标题：</td>
            <td><input  size="40" name="title" value="<?php echo ($newsInfo["title"]); ?>"/></td>
          </tr>
          <tr>
            <td class="changeBack">新闻内容：</td>
            <td><textarea name="content" rows="8" cols="70"><?php echo ($newsInfo["content"]); ?></textarea></td>
          </tr>
            <tr>
	            <td>新闻类型：</td>
	            <td>
	              <select name="typeId">
	       <?php if(is_array($newsTypes)): foreach($newsTypes as $key=>$v): if($v["typeId"] == $oldTypeId): ?><option value="<?php echo ($v["typeId"]); ?>" selected="selected"><?php echo ($v["typeName"]); ?></option>
					<?php else: ?>
				<option value="<?php echo ($v["typeId"]); ?>" ><?php echo ($v["typeName"]); ?></option><?php endif; endforeach; endif; ?>
	              </select>
	            </td>
	          </tr>
	          <tr>
            <td class="changeBack">发送者：</td>
            <td><input size="40" name="userName" value="<?php echo ($newsInfo["userName"]); ?>"/></td>
          </tr>
          <tr>
            <td class="changeBack">新闻作者：</td>
            <td><input size="40" name="writer" value="<?php echo ($newsInfo["writer"]); ?>"/></td>
          </tr>
          <tr>
            <td class="changeBack">新闻来源：</td>
            <td><input size="40" name="source" value="<?php echo ($newsInfo["source"]); ?>"/></td>
          </tr>
          <tr>
            <td class="changeBack">发表日期：</td>
            <td><input size="40" name="dateandtime" value="<?php echo ($newsInfo["dateandtime"]); ?>"/></td>
          </tr>
          <tr>
