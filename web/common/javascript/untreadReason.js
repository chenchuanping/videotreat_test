...
function showRequest(formData, jqForm, options) {
	...
	// $('#myForm').append("<input type='hidden' name='queryForm'
	// value='{"+queryString+"}'/>");
	queryString = '[' + queryString + ']';
	// here we could return false to prevent the form from being submitted;
	// returning anything other than false will allow the form submit to
	// continue
	var currAction = document.getElementById("queryAction").value;
	var queryForm = document.getElementById("queryForm").value;
	var para = "reason=" + queryString + "&" + queryForm;
	var forwardAction = document.getElementById("forwardAction").value;
	$.post(currAction, para, function(data) {
		if (forwardAction != null && forwardAction != "") {
			top.refreshRight(forwardAction);
		} else {
			alert("执行成功");
		}
		// 提交成功后关闭窗口
